...
| `kafka_topic` | 是   | 推送数据的 Kafka 主题（命名规范 `gyhly_{plant_code}`） |
...
遵循 `gyhly_{plant_code}` 格式，全小写，下划线连接：
...
```bash
# 单管道
PIPELINES='[{"mqtt_topic":"device/sis/data","kafka_topic":"gyhly_sd03001","mqtt_broker":"tcp://10.66.87.82","mqtt_port":11883}]' \
...
# 多管道（多电厂）
PIPELINES='[
  {"mqtt_topic":"device/sis/huangdao","kafka_topic":"gyhly_sd02001","mqtt_broker":"tcp://10.0.1.1","mqtt_port":1883,"points_file":"docs/points_huangdao.csv"},
  {"mqtt_topic":"device/sis/dongying","kafka_topic":"gyhly_sd03001","mqtt_broker":"tcp://10.0.2.1","mqtt_port":1883,"points_file":"docs/points_dongying.csv"}
]' go run .
```
...
docker run -d \
  -e PIPELINES='[{"mqtt_topic":"device/sis/data","kafka_topic":"gyhly_sd03001","mqtt_broker":"tcp://10.66.87.82","mqtt_port":11883,"points_file":"/etc/config/points.csv"}]' \
...
```yaml
env:
  PIPELINES: '[{"mqtt_topic":"device/sis/data","kafka_topic":"gyhly_sd03001","mqtt_broker":"tcp://10.66.87.82","mqtt_port":11883,"mqtt_qos":1,"points_file":"/etc/config/points_dongying.csv"}]'
